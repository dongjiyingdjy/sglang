...
def initialize_model_parallel(
    ...
    if attention_context_model_parallel_size * attention_data_parallel_size > tensor_model_parallel_size:
        raise RuntimeError(
            f"attention_context_model_parallel_size ({attention_context_model_parallel_size}) x "
            f"attention_data_parallel_size ({attention_data_parallel_size}) is greater than "
            f"tensor_model_parallel_size ({tensor_model_parallel_size})"
        )

    if expert_model_parallel_size * moe_context_model_parallel_size > tensor_model_parallel_size:
        raise RuntimeError(
            f"expert_model_parallel_size ({expert_model_parallel_size}) x "
            f"moe_context_model_parallel_size ({moe_context_model_parallel_size}) is greater than "
            f"tensor_model_parallel_size ({tensor_model_parallel_size})"
        )
    ...
    if attn_tp_size == tensor_model_parallel_size:
        ...
        _ATTN_TP = init_model_parallel_group(
            ...
            get_world_group().local_rank,
            backend,
            use_pynccl= SYNC_TOKEN_IDS_ACROSS_TP, # TODO
